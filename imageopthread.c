...
void* convolute(){
    ...
    span=src->bpp*src->bpp;
   
    for (row=my_first;row<my_last;row++){
        for (pix=0;pix<src->width;pix++){
            for (bit=0;bit<src->bpp;bit++){
                
                dest->data[Index(pix,row,src->width,bit,src->bpp)]=getPixelValue(src,pix,row,bit,algorithms[type]);
